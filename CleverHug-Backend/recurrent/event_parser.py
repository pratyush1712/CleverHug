...
weekday_codes = ["MO", "TU", "WE", "TH", "FR", "SA", "SU", "MO,TU,WE,TH,FR", "SA,SU"]
...
class RecurringEvent(object):
    ...
    def get_params(self):
        ...
        if self.dtstart:
            params['dtstart'] = self.dtstart.strftime('%Y%m%dT%H%M%S')
        if self.until:
            params['until'] = self.until.strftime('%Y%m%dT%H%M%S')
        elif self.count:
    ...
    def parse_date(self, date_string):
        ...
        if result:
            ...
            return result
        try:
            now_date = datetime.datetime.strptime(self.now_date, '%Y-%m-%dT%H:%M:%S')
            timestruct, result = self.pdt.parse(date_string, now_date)
    ...
    def format(self, rrule_or_datetime):
        ...
        try:
            ...
            print(f'format({rrule_or_datetime}): Exception {e}')
